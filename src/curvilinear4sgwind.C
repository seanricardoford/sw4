...
void curvilinear4sgwind(
  ...
  //
  // Note: Stencil coefficients acof are used at the boundary k=1, coefficients
  // acof_no_gp are used
  //   at the boundary k=nk. In SW4 acof is the stencil coefficient array that
  //   uses ghost points, and acof_no_gp is the stencil coefficients that do not
  //   use ghost points. However, the calling routine can easily control which
  //   boundary stencil is used, for example, if acof is wanted at both
  //   boundaries use acof as actual argument for both formal arguments a_acof
  //   and a_acof_no_gp.
  //
  // opcount:
  ...
  if (op == '=') {
    ...
    RAJA::forall<DEFAULT_LOOP1>(
        RAJA::RangeSegment(0, static_cast<size_t>((ilast - ifirst + 1)) *
                                  (jlast - jfirst + 1) *
                                  (klastw - kfirstw + 1) * 3),
...
//           for (int i = ifirst + 2; i <= ilast - 2; i++) {
      ...
      RAJA::kernel<
        ...
        cof3 = (2 * mu(i, j, k) + la(i, j, k)) * met(2, i, j, k) * strx(i) *
                   met(2, i, j, k) * strx(i) +
               mu(i, j, k) *
        ...
        // 130 ops, tot=1407
        r2 +=
            c2 * (mu(i, j, k + 2) * met(2, i, j, k + 2) * met(1, i, j, k + 2) *
                      (c2 * (u(1, i, j + 2, k + 2) - u(1, i, j - 2, k + 2)) +
                       c1 * (u(1, i, j + 1, k + 2) - u(1, i, j - 1, k + 2))) +
                  (2 * mu(i, j, k + 2) + la(i, j, k + 2)) *
